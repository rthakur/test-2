...
function createTrashWord(trash_words){
		...
		$.each(trash_words,function (){
			 ...
			 html += '<div data-value="'+this.value+'" data-text="'+this.word+'" draggable="true" class="deletedResultWord toBeDragged deleteResultWord"><span>'+this.word+'</span></div>';
		});
...
function getWordCloudCreate(json,canvasid,lightbox){
	...
	$.each(obj, function(key, value){
		...
		if(ignoredKey.indexOf(key.toLowerCase()) === -1){
			...
			var text = {"text":''+key.trim(),"value":value};
